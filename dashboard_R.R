...
## Area --
area <- tabepid[11:14, c(2, 4)] # Seleccionar datos relacionados con area
...
## Hora --
circunstancias <- tabepid[23:25, c(2, 4)] # Seleccionar datos relacionados con cirunstancias
circunstancias$label <- paste0(circunstancias$perc, "%") # Texto de la etiqueta

graphcircunstancias <- ggplot(circunstancias, 
                              aes(x = reorder(Category, desc(perc)), 
...
graphcuerpo <- ggplot(cuerpo, aes(x = reorder(Category, desc(perc)), 
                                  y = perc, fill = Category)) +
  geom_bar(stat = "identity") +
...
a <- raster(xmn=-56.05971, xmx=-53.59184, 
...
pal2 <- colorRampPalette(c("#0571B0", "#92C5DE", "#F7F7F7",
                           "#F4A582", "#CA0020"), interpolate = "linear")(7) 

# Mapa
rm <- leaflet() %>% addTiles() %>%
  addRasterImage(c, colors = pal, opacity = 0.65) %>%
  addLegend(colors = pal2, 
...
smes$perc <- round((smes$`(num = sum(Cantidad))`/sum(smes$`(num = sum(Cantidad))`))*100, 2)
...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    menuItem("Aspectos epidemiológicos", tabName = "epidemio",
             icon = icon("users")),
    menuItem("Plantas hospedantes", tabName = "plantas",
             icon = icon("pagelines")),
    menuItem("Estacionalidad", tabName = "estacionalidad",
             icon = icon("sun")),
    menuItem("Mapa de riesgo", tabName = "mapa",
             icon = icon("map-marked")),
    menuItem("Autoras", tabName = "autoras",
             icon = icon("bookmark"))
    )
)
...
body <- dashboardBody(
  tabItems(
    ...
    tabItem(tabName = "epidemio", h2("Aspectos epidemiológicos"),
            fluidRow(box(h3("Sexo"),
    ...
    tabItem(tabName = "mapa", 
            h2("Mapa de riesgo"),
            p("Mapa de riesgo de lonomismo para Misiones, Argentina 
              (26 de enero de 2014 - 8 de mayo de 2020) usando estimación de 
              densidad kernel."),
            fluidRow(box(leafletOutput("rm"), height = "435px", width = "200px")),
            h2("Número de accidentes por departamento de Misiones"),
            fluidRow(box(tableOutput("tabdep"), 
                         height = "350px", width = "100px"))),
    
    tabItem(tabName = "autoras", 
...
server <- function(input, output) {
  output$graphsex <- renderPlot({graphsex})
  output$graphedad <- renderPlot({graphedad})
  output$grapharea <- renderPlot({grapharea})
  output$graphhora <- renderPlot({graphhora})
  output$graphcircunstancias <- renderPlot({graphcircunstancias})
  output$graphcuerpo <- renderPlot({graphcuerpo})
  output$rm <- renderLeaflet({rm})
  output$tabsp <- function(){tabsp}
  output$grames <- renderPlot({grames})
  output$tabdep <- function(){tabdep}
  }
